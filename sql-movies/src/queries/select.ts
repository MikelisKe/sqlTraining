...
export const selectProductionCompanyById = (id: number): string => {
  return `select id from production_companies where id = ${id}`;
};

export const selectMovie = (imdbId: string): string => {
  return `select id, imdb_id, popularity, budget, budget_adjusted, revenue, revenue_adjusted, original_title, homepage, tagline, overview, runtime, release_date
