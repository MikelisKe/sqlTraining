...
describe("Update Statements", () => {
  ...
  it(
    "should update one app title by app id",
    async (done) => {
      ...
      expect(row.title).toEqual("QUICK VIEW");
      done();
    },
    minutes(1)
  );
  ...
  it(
    "should update review developer reply and developer reply date by app id and author",
    async (done) => {
      const timeStamp = moment().format("YYYY-MM-DD hh:mm");
      const review = await db.selectSingleRow(
        selectReviewByAppIdAuthor(24, "PLAYBOY")
      );
      const query = `UPDATE reviews 
      SET developer_reply = 'test reply', developer_reply_date = '${timeStamp}' 
      ...
      const row = await db.selectSingleRow(
        selectReviewByAppIdAuthor(review.app_id, review.author)
      );
  ...
  it(
    "should update all categories to uppercase",
    async (done) => {
      ...
      const rows = await db.selectMultipleRows(
        selectColumnFromTable("title", CATEGORIES)
      );
