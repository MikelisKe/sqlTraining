...
describe("Simple Queries", () => {
    ...
    it("should select top 3 develepors with most apps published", async done => {
        const query = `select developer, count(*) as count from apps group by developer order by count desc, developer asc limit 3`;
    ...
    it("should select count of reviews created in year 2014, 2015 and 2016", async done => {
        const query = `SELECT COUNT(*) AS review_count, SUBSTR(date_created, 7, 4) AS year
        FROM reviews
